...
than kickoff. A slate-wide tiebreaker (for example predicting the team that scores the most or fewest points across the whole week) can lock at the first game of the slate, earlier than the individual matchups whose scores you also forecast. Preserve source timezone and convert to an unambiguous ISO
...
confirmation timestamp/status. If the provider renders its own saved-completeness indicator (a saved count such as "N of N", a complete/incomplete state, or a tiebreakers-entered flag), record it as corroboration, but still confirm each individual value: the aggregate can read complete while a single field is wrong or stale. A highlighted choice, prior chat, or
