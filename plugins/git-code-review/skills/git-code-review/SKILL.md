...
Run the review against the target diff using the available code review sub-agent/tool. If the host supports `task` agents, invoke `task` with `agent_type: code-review` (this is the built-in reviewer agent, **not** a recursive call to this skill). For changes that touch auth, input handling, secrets, crypto, file/network access, or permissions, **also** use a security review pass against the same diff when available.
